...
-- Strong Entities
CREATE TABLE USERS(userID integer auto_increment, firstName varchar(250),lastName varchar(250),email varchar(250) not null unique,memberSince datetime default NOW(),password varchar(250) not null,dob date,typeOfUser varchar(250),primary key(userID));
...
CREATE TABLE BOOKS(documentID integer, title varchar(250) not null,edition integer not null,publishedOn YEAR, publishedBy varchar(250),primary key(documentID),foreign key(documentID) references DOCUMENTS(documentID));

CREATE TABLE ARTICLES(documentID integer, title varchar(250) not null,journalName varchar(250) not null,journalPublishedOn int, journalPublishedBy varchar(250),primary key(documentID),foreign key(documentID) references DOCUMENTS(documentID));
...
CREATE TABLE SEARCHHISTORY(searchID integer auto_increment,userID integer, documentID integer,searchQuery varchar(250),returnBy datetime,primary key(searchID),foreign key(userID) references USERS(userID),foreign key(documentID) references DOCUMENTS(documentID));
